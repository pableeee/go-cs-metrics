...
func (s *state) onWeaponFire(p demoinfocs.Parser, e events.WeaponFire) {
	...
	row := csraw2.WeaponFire{
		...
		YawDeg:      float32(e.Shooter.ViewDirectionX()),
		PitchDeg:    float32(normalisePitch(float64(e.Shooter.ViewDirectionY()))),
	...
	for _, observer := range players {
		...
		for _, enemy := range players {
			...
			s.match.FirstSights = append(s.match.FirstSights, csraw2.FirstSight{
				...
				ObserverYawDeg:   float32(observer.ViewDirectionX()),
				ObserverPitchDeg: float32(normalisePitch(float64(observer.ViewDirectionY()))),
